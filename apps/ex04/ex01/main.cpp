...
/*   Updated: 2023/09/20 15:59:56 by smorphet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main()
{
    ...
    for (int i = 0; i < NUM_ANIMAL; i++)
    {
        if (animalArray[i])
        {
            delete animalArray[i]; // Deallocate the memory
            animalArray[i] = NULL; // Set the pointer to null to avoid double deletion
        }
    }
