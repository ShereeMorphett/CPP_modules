...
void ClapTrap::beRepaired(unsigned int amount)
{
	if (EnergyPoints_ > 0 && HitPoints_ > 0)
	{
		...
		EnergyPoints_--;
		HitPoints_ += amount;
		std::cout << COLOR_GREEN << "ClapTrap " << name_<< " has "<< HitPoints_ << "HP." << COLOR_RESET << std::endl;
		std::cout << COLOR_GREEN << "ClapTrap " << name_<< " has "<< EnergyPoints_ << "Energy." << COLOR_RESET << std::endl;
	}
	else
	{
		if (EnergyPoints_ <= 0)
			std::cout << COLOR_RED << "ClapTrap " << name_<< " is out of energy and cannot be repaired "<< COLOR_RESET << std::endl;
		else
			std::cout << COLOR_RED << "ClapTrap " << name_<< " has died! It cannot be repaired! "<< COLOR_RESET << std::endl;
	}
