...
/*   Updated: 2023/09/30 13:33:14 by smorphet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::executeForm(AForm const &form) const
{
    if (!form.getSigned())
    {
        std::cout << name_ << " cannot execute " << form.getName()
                  << " because the form is unsigned." << std::endl;
        
    }
    else if (form.getGradeToExecute() < grade_)
    {
        std::cout << name_ << " cannot execute " << form.getName()
                  << " because its grade requirement is too high." << std::endl;
    }
    else
    {
