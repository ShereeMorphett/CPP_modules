...
/*   Updated: 2023/11/15 11:27:30 by smorphet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void PmergeMe::vectorSorting()
{
	...
	else
	{
		...
		while (sortedVec_.size() < inputSize)
		{
			...
			std::cout << "segfault inside while loop" << std::endl;
			...
			while (index > prevJacobshtal)
			{
				std::cout << "segfault inside jacobshtal loop index: " << index << " vec min	" << vecData_[index].min  << " vec max	" << vecData_[index].max  << std::endl;
				vecBinarySearch(vecData_[index].min, vecData_[index].max, sortedVec_);
				index--;
			}
		}
